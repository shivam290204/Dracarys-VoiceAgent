...
<Note>
To hand the caller to **another Dograh agent** rather than to a phone number, use [Transfer to Agent](/voice-agent/tools/transfer-agent) instead. It carries a handover note across the handoff and works on web calls as well as telephony.
</Note>
...
The transfer is a blind transfer. Dograh does not currently pass conversation context to the receiving human as part of the transfer. If you need context to travel with the caller, the destination has to be another Dograh agent — see [Transfer to Agent](/voice-agent/tools/transfer-agent).
