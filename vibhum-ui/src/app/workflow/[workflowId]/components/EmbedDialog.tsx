                ...
                const response = await createOrUpdateEmbedTokenApiV1WorkflowWorkflowIdEmbedTokenPost({
                    ...
                    body: {
                        ...
                        settings: {
                            ...
                            size: "medium",
                            autoStart: false,
                            containerId: embedMode === "inline" ? "vibhum-inline-container" : undefined,
                        },
                        usage_limit: null,
                                        ...
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                ...
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    ...
                                                    <li>• Call <code className="text-xs">window.VibhumWidget.startChat()</code> to start a conversation (the agent greeting arrives via <code className="text-xs">onMessage</code>).</li>
                                                    <li>• Call <code className="text-xs">window.VibhumWidget.sendMessage(text)</code> to send a visitor message; it resolves with the updated transcript, or <code className="text-xs">null</code> if the message could not be delivered.</li>
                                                    <li>• Call <code className="text-xs">window.VibhumWidget.endChat()</code> to end the active conversation and trigger its completion webhook.</li>
                                                    ...
                                                    <li>• Call <code className="text-xs">window.VibhumWidget.setContext({"{ ... }"})</code> before <code className="text-xs">startChat()</code> to pass visitor details the page learned after load.</li>
                                                </ul>
                                            </div>
                                        ...
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                ...
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    ...
                                                    <li>• Call <code className="text-xs">window.VibhumWidget.start()</code> to begin a call.</li>
                                                    <li>• Call <code className="text-xs">window.VibhumWidget.end()</code> to end it.</li>
                                                    ...
                                                    <li>• Call <code className="text-xs">window.VibhumWidget.setContext({"{ ... }"})</code> before <code className="text-xs">start()</code> to pass visitor details the page learned after load.</li>
                                                </ul>
                                            </div>
...
window.VibhumWidget?.onStatusChange((status) => {
  callStatus = status;
...
document.getElementById('talk-btn').addEventListener('click', () => {
  if (callStatus === 'connected' || callStatus === 'connecting') {
    window.VibhumWidget.end();
  } else {
    window.VibhumWidget.start();
  }
});`}</code>
                                                </pre>
  ...
  useEffect(() => {
    window.VibhumWidget?.onStatusChange(setStatus);
  }, []);

  const isLive = status === 'connected' || status === 'connecting';
  return (
    <button onClick={() => isLive ? window.VibhumWidget.end() : window.VibhumWidget.start()}>
                                    ...
                                    {embedMode === "inline" && (
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                <h4 className="font-medium mb-2">Integration Instructions</h4>
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    <li>• Add a div with id=&quot;vibhum-inline-container&quot; where you want the widget</li>
                                                    ...
                                                    {widgetType === "chat" ? (
                                                        ...
                                                        <>
                                                            <li>• Call window.VibhumWidget.start() to begin the call</li>
                                                            <li>• Call window.VibhumWidget.end() to end the call</li>
                                                        </>
                                                    )}
                                                </ul>
                                            </div>
...
<div id="vibhum-inline-container" style="min-height: 480px">
  ...
  useEffect(() => {
    ...
    window.VibhumWidget?.onCallStart(() => {
      setIsCallActive(true);
    });
    window.VibhumWidget?.onCallEnd(() => {
  ...
  return (
    <div className="my-8">
      ...
      <div id="vibhum-inline-container" className="min-h-[400px]">
        {/* Widget renders here */}
      </div>
      <button
        onClick={() => window.VibhumWidget?.start()}
                                    ...
                                    <>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                ...
                                                <code className="text-xs">{"window.VibhumWidget.setContext({ ... })"}</code> for
