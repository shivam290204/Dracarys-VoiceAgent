...
export function AppSidebar() {
  ...
  // Admin role check for Sidebar items
  const adminEmails = process.env.NEXT_PUBLIC_ADMIN_EMAILS || '';
  const isUserAdmin = !adminEmails || adminEmails === '*' || (user?.email && adminEmails.split(',').map(e => e.trim().toLowerCase()).includes(user.email.toLowerCase()));

  const adminRoutes = ['/campaigns', '/model-configurations', '/telephony-configurations', '/tools', '/api-keys'];

  const visibleSections = NAV_SECTIONS.map(section => ({
    ...section,
    items: section.items.filter(item => isUserAdmin || !adminRoutes.some(r => item.url === r || item.url.startsWith(`${r}/`)))
          ...
          <div className={cn("flex items-center gap-2", isCollapsed && "hidden")}>
            ...
            {isBehind && latestRelease && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <a
                    href="https://docs.vibhum.ai/deployment/update"
                    target="_blank"
