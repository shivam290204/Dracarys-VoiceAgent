...
from api.services.business_hours import get_business_hours, upsert_business_hours
...
@router.get("/business-hours", response_model=BusinessHoursConfiguration)
async def get_business_hours_route(
    user: UserModel = Depends(get_user_with_selected_organization),
):
    organization_id = user.selected_organization_id
    return await get_business_hours(organization_id)


@router.put("/business-hours", response_model=BusinessHoursConfiguration)
async def save_business_hours_route(
    request: BusinessHoursConfiguration,
    user: UserModel = Depends(get_user_with_selected_organization),
):
    organization_id = user.selected_organization_id
    return await upsert_business_hours(
        organization_id,
        request,
    )
