...
from api.services.business_hours import get_business_hours
...
async def handle_inbound_run(request: Request):
    ...
    try:
        ...
        # --- AFTER-HOURS ROUTING ---
        business_hours = await get_business_hours(config.organization_id)
        if business_hours.enabled and business_hours.after_hours_workflow_id:
            try:
                tz = zoneinfo.ZoneInfo(business_hours.timezone)
                now = datetime.now(tz)
                day_name = now.strftime("%A").lower()
                current_time_str = now.strftime("%H:%M")
                
                # Check if current time falls into any slot for today
                is_open = False
                slots = getattr(business_hours.schedule, day_name, [])
                for slot in slots:
                ...
                if not is_open:
                    logger.info(f"/inbound/run: Outside business hours. Routing to {business_hours.after_hours_workflow_id}")
                    workflow_id = business_hours.after_hours_workflow_id
            except Exception as e:
                logger.error(f"/inbound/run: Failed to check business hours: {e}")
        # ---------------------------
        workflow = await db_client.get_workflow(
